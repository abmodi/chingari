...
case class MaxExpression(expr : Expression) extends AggregateExpression {
  override def inputExpression(): Expression = expr

  override def createAccumulator(): Accumulator = MaxAccumulator()
}

case class MaxAccumulator() extends Accumulator {
